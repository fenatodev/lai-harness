...
class IsolatedInstallSmokeTest(unittest.TestCase):
    ...
    def test_model_server_start_auto_discovers_windows_launcher_without_secret_values(self):
        ...
        self.assertIn('load_lai_model_defaults', source)
        self.assertIn('LAI_MODEL_DEFAULTS_FILE:-$HOME/.config/lai/model.env', source)
        self.assertIn('LAI_GPU_LAYERS|LAI_PARALLEL|LAI_LLAMA_SERVER', source)
        self.assertNotIn('source "$defaults_file"', source)
