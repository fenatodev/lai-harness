...
class IsolatedInstallSmokeTest(unittest.TestCase):
    def test_install_doctor_sample_repo_and_deterministic_commands(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(workspace_payload["version"], "0.4.0-beta.20")
            ...
            self.assertEqual(readiness_payload["version"], "0.4.0-beta.20")
            ...
            self.assertEqual(release_payload["version"], "0.4.0-beta.20")
            ...
            self.assertEqual(release_pack_payload["version"], "0.4.0-beta.20")
            ...
            self.assertEqual(governance_payload["version"], "0.4.0-beta.20")
            ...
            self.assertEqual(alias_governance_payload["version"], "0.4.0-beta.20")
            ...
            self.assertEqual(project_handoff_payload["version"], "0.4.0-beta.20")
            self.assertTrue(Path(project_handoff_payload["files"]["markdown"]).is_file())

            next_chat = subprocess.run(
                [str(bin_dir / "lai"), "next-chat", "--target", "0.4.0-beta.20", "--json"],
                cwd=sample_repo,
                env=install_env,
                text=True,
                capture_output=True,
            ...
            self.assertEqual(next_chat_payload["version"], "0.4.0-beta.20")
