...
class LocalAgentTest(unittest.TestCase):
    ...
    def test_run_history_lists_shows_tails_and_exports_recorded_runs(self):
        ...
        self.assertEqual(payload["version"], "0.4.0-beta.10")
    ...
    def test_release_preflight_context_prefers_project_commands(self):
        ...
        self.assertIn("Version: 0.4.0-beta.10", context)
    ...
    def test_release_check_is_deterministic_and_read_only(self):
        ...
        result = subprocess.run(
            [str(SOURCE), "--release-check", "--target", "0.4.0-beta.10", "--json"],
            cwd=self.root,
            env=env,
            text=True,
            capture_output=True,
        ...
        self.assertEqual(payload["version"], "0.4.0-beta.10")
        self.assertEqual(payload["expected_tag"], "v0.4.0-beta.10")
    ...
    def test_release_check_distinguishes_integration_and_tag_safety(self):
        ...
        def fake_git(args, default="[unavailable]", timeout=10):
            ...
            if args == ["rev-parse", "v0.4.0-beta.10^{}"]:
        ...
        with mock.patch.object(agent, "collect_readiness_status", return_value=readiness), \
            ...
            candidate = agent.collect_release_check("0.4.0-beta.10")
            ...
            taggable = agent.collect_release_check("0.4.0-beta.10")
            ...
            divergent = agent.collect_release_check("0.4.0-beta.10")
            ...
            wrong_tag = agent.collect_release_check("0.4.0-beta.10")
            ...
            state["exact_tag"] = "v0.4.0-beta.10"
            released = agent.collect_release_check("0.4.0-beta.10")
    ...
    def test_release_pack_writes_local_files_without_repo_mutation(self):
        ...
        self.assertIn("git tag -a v0.4.0-beta.10", commands)
        self.assertIn("git pull --ff-only origin main", commands)
        self.assertIn("git push origin v0.4.0-beta.10", commands)
    ...
    def test_release_governance_reports_manual_publication_actions(self):
        ...
        rendered = subprocess.run(
            [str(SOURCE), "--release-governance", "--target", "0.4.0-beta.10"],
            cwd=self.root,
            env=env,
            text=True,
            capture_output=True,
    ...
    def test_release_governance_parses_remote_and_github_origins(self):
        options = agent.parse_release_governance_args([
            "--target", "0.4.0-beta.10", "--remote", "--json",
        ])
        self.assertEqual(options["target"], "0.4.0-beta.10")
    ...
    def test_remote_release_verifies_optional_vsix_digest(self):
        vsix = self.base / "lai-harness-0.4.0-beta.10.vsix"
        ...
        remote = {
            ...
            "draft": False,
            "prerelease": True,
            "html_url": "https://github.com/fenatodev/lai-harness/releases/tag/v0.4.0-beta.10",
            "assets": [{
                "name": vsix.name,
    ...
    def test_remote_governance_clears_verified_github_actions(self):
        release_payload = {
            "overall": "ready",
            "phase": "released",
            "target_version": "0.4.0-beta.10",
            "expected_tag": "v0.4.0-beta.10",
            "branch": "main",
            "head": "abc123",
            "latest_tag": "v0.4.0-beta.10",
            "exact_tag": "v0.4.0-beta.10",
            "origin": "git@github.com:fenatodev/lai-harness.git",
        }
        pack = {
            "status": "ok",
            ...
            "missing": [],
            "required_files": {},
            "vsix": {"path": "/tmp/pack/lai-harness-0.4.0-beta.10.vsix", "present": False, "optional": True},
        }
        remote = {
            "enabled": True,
        ...
        with mock.patch.object(agent, "collect_release_check", return_value=release_payload), \
            ...
            result = agent.collect_release_governance(target="0.4.0-beta.10", remote=True)
    ...
    def test_project_handoff_remote_converges_live_release_state(self):
        ...
        pack = {
            "status": "ok",
            "pack_dir": "/tmp/lai-harness-release-pack-v0.4.0-beta.10",
            "vsix": {"path": "/tmp/lai-harness-0.4.0-beta.10.vsix", "present": True},
        }
        local_governance = {
        ...
        with mock.patch.object(agent, "collect_release_check", return_value=release_payload), \
            ...
            payload = agent.project_handoff_payload("0.4.0-beta.10", remote=True)
        ...
        self.assertIn("project-handoff --target 0.4.0-beta.10 --remote --json", markdown)
        self.assertEqual(
        ...
        with mock.patch.object(agent, "collect_release_check", return_value=release_payload), \
                ...
                mock.patch.object(agent, "collect_safe_workspace_status", return_value={"base_dir": "/tmp/workspaces", "workspaces": []}), \
                mock.patch.object(agent, "load_active_spec", return_value=None):
            offline = agent.project_handoff_payload("0.4.0-beta.10", remote=False)
        self.assertFalse(offline["release_governance_remote_enabled"])
        self.assertIsNone(offline["release_governance_remote_overall"])
        offline_governance.assert_called_once_with(target="0.4.0-beta.10", remote=False)
    ...
    def test_project_handoff_renders_and_writes_next_chat_reference(self):
        ...
        rendered = subprocess.run(
            [str(SOURCE), "--project-handoff", "--target", "0.4.0-beta.10"],
            cwd=self.root,
            env=env,
            text=True,
            capture_output=True,
    ...
    def test_deterministic_version_command_needs_no_server(self):
        ...
        self.assertEqual(result.stdout.strip(), "lai harness 0.4.0-beta.10")
    ...
    def test_public_publishing_surface_uses_current_beta_identity(self):
        ...
        self.assertIn("lai harness v0.4.0-beta.10", publishing)
        self.assertIn("lai harness v0.4.0-beta.10", release_notes)
        self.assertIn("v0.4.0-beta.10", release_checklist)
        self.assertIn("lai release-pack", release_pack)
        self.assertIn("lai-harness-0.4.0-beta.10.vsix", release_pack)
