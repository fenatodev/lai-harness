...
> **Current release:** `v0.4.6` · stable recovery rollback · Linux/WSL-first · local inference through an OpenAI-compatible endpoint such as llama.cpp.
...
- repository-scoped persistent-session lifecycle commands with `lai sessions`, `lai sessions show <session-id>`, and `lai sessions delete <session-id>`;
