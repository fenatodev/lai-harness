...
- deterministic `lai policy-check` for non-executing policy classification and repository shell-hook reuse;
- repository development hooks plus an L4 Harness Score CI ratchet, separate from product runtime CI;
...
- authenticated loopback-only `lai serve` control plane with serialized asynchronous `plan`/`review`/`security`/`diagnose`/`release` runs under explicit shell-free remote capability profiles, bounded cancellation/output, and no HTTP shell/write execution;
...
@lai /release verify whether beta.13 is ready
