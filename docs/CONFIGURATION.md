...
Sandboxed work-runs set `LAI_SKILLS_DIR=/workspace/skills` and receive a generated `LAI_CONTROL_MODEL_BRIDGE_SOCKET` value that points at `/workspace/.lai-model-bridge.sock`. Operators should not set this variable manually; it is a per-run supervisor contract for model access without container networking.
