...
```bash
lai release-pack --target 0.4.0-beta.23
lai release-pack --target 0.4.0-beta.23 --json
lai release-pack --target 0.4.0-beta.23 --with-vsix
lai release-pack --target 0.4.0-beta.23 --out /tmp/lai-harness-release-pack-v0.4.0-beta.23
```

By default the pack is written outside the repository under:

```text
/tmp/lai-harness-release-pack-v0.4.0-beta.23
```
...
- `lai-harness-0.4.0-beta.23.vsix` — only when `--with-vsix` is used.
...
Push the feature branch and integrate it through a green PR into protected `main`. After main CI passes, sync `main`, create/push only `v0.4.0-beta.23`, verify tag CI, and then create the GitHub Release as a pre-release.
