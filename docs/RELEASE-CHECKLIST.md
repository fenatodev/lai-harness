...
make check
make validate
./scripts/package-vsix.sh /tmp/lai-harness-0.4.0-beta.2.vsix
```

Expected posture before tagging:

- `lai readiness` reports `Overall: ready`.
- `lai release-check --target 0.4.0-beta.2 --json` reports `phase=ready_to_tag`.
...
```bash
git tag -a v0.4.0-beta.2   -m "v0.4.0-beta.2 — release polish"

git switch main
git merge --ff-only feature/v0.4.0-beta.2-release-polish

git push --atomic origin main v0.4.0-beta.2
```
...
- `v0.4.0-beta.2` is an annotated tag pointing to the same commit.
- GitHub Actions passes for `main`.
- GitHub Actions passes for `v0.4.0-beta.2`.

## Optional GitHub Release

After CI is green:

1. Create a GitHub Release from tag `v0.4.0-beta.2`.
2. Use the title from [GitHub publishing metadata](GITHUB-PUBLISHING.md).
3. Paste the body from [Release notes](RELEASE-NOTES.md).
4. Attach the inspected `/tmp/lai-harness-0.4.0-beta.2.vsix` only if you want to distribute a manual VSIX artifact.
