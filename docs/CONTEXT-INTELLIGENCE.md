...
LAI uses a small deterministic ranking layer to reduce repository-discovery rounds for constrained local models. It does not replace `read`, `search`, or `inspect`, and ranked files are never treated as already inspected evidence.
...
LAI does not use embeddings, vector databases, external indexing services, MCP, delegates, or learning for this feature. Rankings are recomputed from current local evidence and are not persisted as a separate index.
