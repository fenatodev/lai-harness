...
Use `lai release-pack --target 0.4.0-beta.24 --with-vsix --json` to generate local publication files.

**Expected title:** lai harness v0.4.0-beta.24 — semantic contract modularization

The generated `release-body.md` must come from the exact `## lai harness v0.4.0-beta.24` section in [Release notes](RELEASE-NOTES.md).

Before merge, protected `main` must require `Python 3.11`, `Python 3.12`, `Publication gates`, and `Harness Score L4`.

After publication, run `lai release-governance --target 0.4.0-beta.24 --remote --json` to verify branch protection, pre-release metadata, and the VSIX digest when attached.
