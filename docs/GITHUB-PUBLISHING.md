...
## Release v0.4.0-beta.12

Use `lai release-pack --target 0.4.0-beta.12 --with-vsix --json` to generate local release files.

**Title:** lai harness v0.4.0-beta.12 — asynchronous read-only control runs
...
After publication, run `lai release-governance --target 0.4.0-beta.12 --remote --json` to verify branch protection, pre-release metadata, and the VSIX digest when attached.
