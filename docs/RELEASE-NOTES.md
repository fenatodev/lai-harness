...
- Added optional `~/.config/lai/model.env` support to `lai-server-start` so local GPU/context defaults can persist across `stack-start` without hardcoding machine-specific settings in public scripts.
- Count child-run modified/recent paths with workspace-local sanitization so telemetry summaries do not drop valid workspace writes or count runtime paths.
